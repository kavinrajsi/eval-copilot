...
>
> Two more shipped extensions keep the same shape. **The computer's side grew** two text-similarity rules (`rouge_l`, `jaccard`) — still exact, deterministic comparisons against the known-good. **Judge mode can score multiple criteria** (each 0–100, overall vs a pass threshold) — richer output, but still a first pass a human can override. And the **confusion matrix** in Results is "check the checker" made visible: it plots the machine verdict against the human's and surfaces the false-pass rate directly, so the person's-side disagreement named below is measured, not hidden.
